...
sudo apt-get install -y curl ca-certificates tzdata perl jq zip
...
sudo EXTERNAL_URL=$EXTERNAL_URL dpkg -i $GITLAB_BACKUP_VERSION.0_amd64.deb
...
unzip -P $GITLAB_BAKCUP_PASSOWRD backup_archived.zip -d backup_extracted
...
# Restore from backup
BACKUP_FILENAME=$(ls backup_extracted/*_gitlab_backup.tar)
...
gitlab-rake gitlab:backup:restore BACKUP=$(echo $BACKUP_FILENAME | cut -d "_" -f 1-5)
gitlab-ctl start
...
# TBD Seed shared runners registartion token
